...
import { nameOfBlock, blocksCount, indexOfBlock, getSolidBlocksStatus } from './blockinfo.js';
...
export default class ModelData {
    ...
    constructor(size, height = 0) {
        ...
        this.solidBlocks = getSolidBlocksStatus();
    ...
    static load(jsonOrString) {
        ...
        model.convertMap(uint8, info.blockNames);
    ...
    dump() {
        const info = {
            ...
            blockNames: this.listOfBlocks()
        }

        return JSON.stringify(info, null, 2);
    }

    convertMap(newMap, blockNames) {
        // check if need to convert
        let convert = false;
        if (blockNames && blockNames.length > 0) {
            for (let i = 0 ; i < blockNames.length ; i++) {
                if (nameOfBlock(blockNames[i][0]) !== blockNames[i][1])
                    convert = true;
            }
        } else
            convert = true;
        ...
        if (!convert) {
            for (let i = 0 ; i < newMap.length ; i++) {
                this.data[i] = newMap[i];
            }
            return;
        }

        // conversion table
        let conv = [], lastId = blockNames[blockNames.length - 1][0];
        for (let i = 0 ; i <= lastId ; i++) {
            conv.push(i);
        }
        const maxid = blocksCount() - 1;
        blockNames.forEach(item => {
            let newIdx = indexOfBlock(item[1]);
            if (newIdx < 0) {
                newIdx = item[0] >= maxid ? maxid : item[0];
                console.warn(`Unknown block "${item[1]}" (old id: ${item[0]})`);
            }
        ...
        // convert
        for (let i = 0 ; i < newMap.length ; i++) {
            this.data[i] = conv[newMap[i]];
        }
    }

    listOfBlocks() {
        const list = [];
        const count = blocksCount();
        ...
        for (let p = 0 ; p < this.data.length ; p++) {
            const id = this.data[p];
            if (!id || list[id])
                continue;
            
            list[id] = [id, nameOfBlock(id)];
        }
    ...
    countNeighbors(pos) {
        ...
        const solid = this.solidBlocks;


        if (pos.x - 1 < 0 || pos.x - 1 >= size || solid[this.data[idx - 1]])
            count++;
        if (pos.x + 1 < 0 || pos.x + 1 >= size || solid[this.data[idx + 1]])
            count++;

        if (pos.z - 1 < 0 || pos.z - 1 >= size || solid[this.data[idx - size]])
            count++;
        if (pos.z + 1 < 0 || pos.z + 1 >= size || solid[this.data[idx + size]])
            count++;

        if (pos.y - 1 < 0 || pos.y - 1 >= this.height || solid[this.data[idx - ysize]])
            count++;
        if (pos.y + 1 < 0 || pos.y + 1 >= this.height || solid[this.data[idx + ysize]])
            count++;
